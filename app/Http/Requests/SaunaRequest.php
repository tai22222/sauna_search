...
class SaunaRequest extends FormRequest
{
    ...
    public function rules(FormRequest $request)
    {
      ...
      if($this->isMethod('post') && $this->routeIs('saunas.store')){
        logger("==========SaunaControllerのstoreアクション=============");
      ...
      if($this->isMethod('put')){
        logger("==========SaunaControllerのupdateアクションのput=============");

        return $commonRules;
      }

      
      // サ活の新規投稿のバリデーション(/saunas/{id}からpost)
      if($this->isMethod('post') && $this->routeIs('review.store')){
        logger("==========ReviewControllerのstoreアクション=============");

        $postRules =[
          'visited_date' =>  ['nullable', 'string'],
          'title' =>  ['nullable', 'string', 'max:255'],
          'content' =>  ['nullable', 'string', 'max:1024'],
          'review_image' => ['nullable', 'mimetypes:image/jpeg,image/png', 'max:1024'],
        ];
      ...
      if($this->isMethod('post')){
        ...
        logger("==========SaunaControllerのupdateアクションのpost(画像データ)=============");
        $postRules = [
          // 画像アップロード(一旦String型に設定)
